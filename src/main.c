...
void tvbgone_sleep( void )
{
  ...
  PORTA &= ~_BV(LED);       // turn off visible LED
  PORTB &=~_BV(IRLED);     // turn off IR LED
...
void quickflashLED( void ) {
  PORTA |= _BV(LED);   // turn on visible LED at PB0 by pulling pin to ground
  delay_ten_us(3000);   // 30 millisec delay
  PORTA &= ~_BV(LED);    // turn off visible LED at PB0 by pulling pin to +3V
}
